...
RESULTS_BASE="$SCRIPT_DIR/results/lora_retriever_eval_ood"

MODEL="qwen2b"
GPU_ID="2"
APPS=(adidas calendar decathlon gmail google_maps kitchen_stories)
METHODS=(fusion mixture)
LORA_TYPE="app"
ORIG_ARGS=("$@")

# bash evaluation/run_eval_ood.sh --model qwen2b --gpu_id 0
# nohup bash evaluation/run_eval_ood.sh --model qwen2b --lora_type app --gpu_id 0 > evaluation/run_eval_ood_qwen2b.nohup.log 2>&1 &
# nohup bash evaluation/run_eval_ood.sh --model qwen7b --lora_type app --gpu_id 6 > evaluation/run_eval_ood_qwen7b.nohup.log 2>&1 &
usage() {
  cat <<USAGE
Usage:
  bash evaluation/run_eval_ood.sh [options]

Options:
  --model MODEL        qwen2b|qwen7b|intern2b (default: qwen2b)
  --gpu_id ID          GPU id (default: 0)
  --gou_id ID          Alias of --gpu_id
  --lora_type TYPE     all|app|category (default: app)
...
case "$LORA_TYPE" in
  all|app|category) ;;
  *)
    echo "[ERROR] Unsupported --lora_type: $LORA_TYPE (allowed: all app category)"
    exit 1
    ;;
esac

if [[ "$LORA_TYPE" != "app" ]]; then
  echo "[ERROR] run_eval_ood.sh is app-only. Use --lora_type app. Got: $LORA_TYPE"
  exit 1
fi
...
RUN_ID="${TS}_pid$$"
SUMMARY_FILE="$RESULTS_BASE/summary_4apps_${RUN_ID}.tsv"
...
echo "============================================================"
echo "[RUN_META] run_id=$RUN_ID pid=$$ ppid=$PPID ts=$TS args=${ORIG_ARGS[*]}"
